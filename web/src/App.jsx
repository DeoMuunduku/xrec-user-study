...
const TIME_CONDITIONS = ["t8", "t16", "full"];
const STORAGE_KEY_TIME = "study_time_condition_v10";
...
function getReadSeconds(cond) {
  if (cond === "t8") return 8;
  if (cond === "t16") return 16;
  return null; // full = self-paced
}

function getK(cond) {
  if (cond === "t8") return 3;
  if (cond === "t16") return 6;
  return 8;
}
...
async function submitStudy(payload) {
  ...
  // ✅ OPTION A: endpoint /submit (pas /submit)
...
/** cueSet = chosen + completed by generic order to reach K */
...
/** aligned: selected cues first (importance order), then completed cues */
...
/** baseline: generic order filtered to cueSet */
...
function WelcomeScreen({ onNext, conditionText, debugLabel }) {
  return (
    <div className="screen welcome-screen">
      <div className="card welcome-card">
        ...
        <div className="scenario-box">
          <h3>Scénario</h3>
          <p>
            Imaginez que vous êtes dans une <strong>ville ou un pays étranger</strong> et que vous souhaitez aller dans
            un restaurant.
          </p>
          <p>
            Pour vous aider, notre système vous demande d’indiquer vos <strong>préférences</strong>, puis il recommande
            un restaurant <strong>et l’explique</strong>.
          </p>

          <h3 style={{ marginTop: 14 }}>Votre rôle</h3>
          <p>
            Vous allez lire <strong>deux explications</strong> (séparées), puis répondre à quelques questions après
            chacune.
          </p>

          <div
            className="warning-box"
            style={{ marginTop: 12, borderColor: "#ef4444", color: "#b91c1c", fontWeight: 900 }}
          >
...
function PreferencesScreen({ prefMap, setPrefMap, onNext, onBack }) {
  ...
  const canContinue = useMemo(() => {
    return selectedPrefs.length >= MIN_PREFS_ALL && allSelectedAreComplete;
    ...
    <div className="screen priority-screen">
      <div className="card card-wide">
        ...
        <p className="screen-description">
          Sélectionnez au moins <strong>{MIN_PREFS_ALL}</strong> préférences. Pour chaque préférence sélectionnée,
          indiquez la préférence et l’importance.
        </p>
              ...
              <div key={cue.id} className={`pref-row${rowDisabled ? " disabled" : ""}`}>
                <div className="pref-crit">
                  <input
                    ...
                    onChange={() => toggleSelected(cue.id)}
                    aria-label={`Sélectionner ${cue.label}`}
                  />
                  <span>{cue.label}</span>
                </div>
        ...
        {!canContinue && (
          <div className="warning-box" style={{ marginTop: 14 }}>
            ...
            <div style={{ marginTop: 6, opacity: 0.8, fontSize: 13 }}>
              Sélection actuel : {selectedPrefs.length} / {MIN_PREFS_ALL}
            </div>
          </div>
        )}
...
/* =========================================================
   PAGE 3 : READING + QUESTIONS (A then B)
   ========================================================= */
...
function ReadingScreen({
  ...
  const gistOk =
    (cur.gist?.length || 0) === 2 || ((cur.gist?.length || 0) === 1 && cur.gist?.[0] === "other");
        ...
        {phase === "prepare" && (
          <div style={{ marginTop: 12 }}>
            ...
            {readSeconds != null ? (
              ...
              <div style={{ marginTop: 10, fontWeight: 900 }}>
                Vous pourrez lire l’explication, puis répondre à quelques questions.
              </div>
            )}
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              <div>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>
                  1) Avec cette explication, iriez-vous dans ce restaurant ?
                </div>
                <div className="comparison-options" style={{ justifyContent: "flex-start" }}>
                  {[
                    { id: "yes", label: "Oui" },
                    { id: "no", label: "Non" },
                    { id: "unsure", label: "Incertain" },
                  ].map((opt) => (
              ...
              <div>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>
                  2) À quel point êtes-vous sûr(e) de votre décision ?
                </div>
              ...
              <div>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>
                  3) L’explication commençait par ce qui est le plus important pour moi.
                </div>
                <div className="likert-scale">
                  {[1, 2, 3, 4, 5].map((val) => (
                    <button
              ...
              <div style={{ borderTop: "1px solid rgba(0,0,0,0.08)", paddingTop: 10 }}>
                <div style={{ fontWeight: 700, marginBottom: 6 }}>
                  4) Quels sont les 2 points principaux que vous retenez ? (choisissez 2)
                </div>
              ...
              <div>
                ...
                <select
                  className="pref-select"
                  value={cur.coverage}
                  onChange={(e) => setCur({ coverage: e.target.value })}
                >
            ...
            {!curComplete && (
              <div style={{ marginTop: 10, fontSize: 13, opacity: 0.75 }}>
                À compléter : décision, confiance, “commence par…”, gist (2 points), couverture.
              </div>
            )}
          </div>
        )}
...
function ThankYouScreen({ data }) {
  const label = data?.timeCondition === "t8" ? "8s" : data?.timeCondition === "t16" ? "16s" : "full";
  return (
    <div className="screen thankyou-screen">
      <div className="card welcome-card">
        <h2>Merci pour votre participation !</h2>
        <p>Vos réponses ont été enregistrées.</p>
        <p style={{ opacity: 0.75 }}>Condition : {label}</p>
...
export default function App() {
  ...
  const [prefMap, setPrefMap] = useState(() => {
    const obj = {};
    CUES.forEach((c) => {
      obj[c.id] = { ...c, selected: false, preference: "", importance: "" };
    });
    return obj;
  });
  ...
  const expTexts = useMemo(() => {
    ...
    return {
      A: config.alignedIsA ? alignedText : baselineText,
      B: config.alignedIsA ? baselineText : alignedText,
    };
  ...
  return (
    <div className="app-container">
      {screen === 0 && (
        <WelcomeScreen conditionText={conditionText} debugLabel={debugLabel} onNext={() => goto(1)} />
      )}

      {screen === 1 && (
        <PreferencesScreen prefMap={prefMap} setPrefMap={setPrefMap} onBack={() => goto(0)} onNext={() => goto(2)} />
      )}
